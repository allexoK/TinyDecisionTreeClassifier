...
#define TRREE_TRAINING_TIME_PER_CLASS_SECONDS       120
#define TRREE_TEST_TIME_PER_CLASS_SECONDS           (uint32_t)(TRREE_TRAINING_TIME_PER_CLASS_SECONDS/4)
#define TREE_NUMBER_OF_CLASSES                      3
#define TREE_NUMBER_OF_FEATURES                     9
#define TREE_NUMBER_OF_TRAINING_SAMPLES   (uint32_t(TREE_NUMBER_OF_CLASSES*TRREE_TRAINING_TIME_PER_CLASS_SECONDS/WINDOW_SIZE_SECONDS))+1
#define TREE_NUMBER_OF_TEST_SAMPLES       (uint32_t(TREE_NUMBER_OF_CLASSES*TRREE_TEST_TIME_PER_CLASS_SECONDS/WINDOW_SIZE_SECONDS))+1
...
uint32_t before = 0;
uint32_t after = 0;
Treetype Xp[TREE_NUMBER_OF_FEATURES];
float accuracy=0;
void loop() {
  for(uint32_t k=0;k<TREE_NUMBER_OF_CLASSES;k++){
    Serial.print("Recording train data for class:");
    Serial.println(k);
    for(uint32_t i=0;i<(TRREE_TRAINING_TIME_PER_CLASS_SECONDS/WINDOW_SIZE_SECONDS);i++){
      for(uint32_t j=0;j<SAMPLING_FREQENCY*WINDOW_SIZE_SECONDS;j++){
        mpu6050.update();
    ...
    for(uint32_t i=0;i<(TRREE_TEST_TIME_PER_CLASS_SECONDS/WINDOW_SIZE_SECONDS);i++){
      for(uint32_t j=0;j<SAMPLING_FREQENCY*WINDOW_SIZE_SECONDS;j++){
        mpu6050.update();
        aXbuffer[j]=mpu6050.getRawAccX();
        aYbuffer[j]=mpu6050.getRawAccY();
